...
// sat
std::pair<std::shared_ptr<arcade::ir::Node>, std::shared_ptr<arcade::ir::VariableManager>> example39() {
    using namespace arcade::ir;
    auto variable_manager = std::make_shared<VariableManager>();
    // global variables
    auto k = variable_manager->AddGlobalIntegerVariable("k", 100, AccessType::kWrite);
    auto b = variable_manager->AddGlobalBooleanVariable("b", false, AccessType::kWrite);
    ...
    CfaBuilder builder;
    {
        ...
        handle->AddAssignment(i, j);
        handle->AddAssignment(n, 0);
        handle->AddAssignment(b, fd);
    ...
    {
        auto handle = builder.AddInitialLocation();
        auto [then_handle, else_handle] = handle->AddIf(n < 2 * k);
        {
    ...
    {
        auto handle = builder.AddInitialLocation();
        auto [then_handle, else_handle] = handle->AddIf(b);
        {
            then_handle->AddAssignment(i, i + 1);
        }
        {
            else_handle->AddAssignment(j, j + 1);
        }
        handle->AddAssignment(b, !b);
        handle->AddAssignment(n, n + 1);
