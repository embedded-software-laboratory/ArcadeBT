...
// unsat
std::pair<std::shared_ptr<arcade::ir::Node>, std::shared_ptr<arcade::ir::VariableManager>> example15() {
    using namespace arcade::ir;
    auto variable_manager = std::make_shared<VariableManager>();
    // global variables
    auto p = variable_manager->AddGlobalIntegerVariable("p", 1, AccessType::kWrite);
    ...
    auto builder = CfaBuilder();
    {
        ...
        auto [then_handle, else_handle] = handle->AddIf((count <= 6) & r2);
        then_handle->AddReturn(ReturnValue::kSuccess);
        else_handle->AddAssignment(p, p + 2);
