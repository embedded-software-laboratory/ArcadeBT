...
// sat
std::pair<std::shared_ptr<arcade::ir::Node>, std::shared_ptr<arcade::ir::VariableManager>> example25() {
    ...
    auto builder = CfaBuilder();
    {
        ...
        handle->AddAssignment(i, 1);
        handle->AddAssignment(j, 0);
        handle->AddAssignment(z, i - j);
        handle->AddAssignment(x, 0);
        handle->AddAssignment(y, 0);
        handle->AddAssignment(w, 0);
    ...
    {
        auto handle = builder.AddInitialLocation();
        handle->AddAssignment(z, z + x + y + w);
        handle->AddAssignment(y, y + 1);
        auto [then_handle, else_handle] = handle->AddIf(z % 2 == 1);
        {
            then_handle->AddAssignment(x, x + 1);
        }
        
        handle->AddAssignment(w, w + 2);
